...
export default function AdminUsers() {
    ...
    const handleSaveUser = async (userData: Partial<User>) => {
        ...
        try {
            ...
            if (!response.ok) throw new Error(`Failed to ${modalMode === 'create' ? 'create' : 'update'} user`)
                            ...
                            <tbody className="divide-y divide-white/5">
                                {paginatedUsers.map((user) => (
                                    <tr key={user.id} className="hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${user.is_superuser ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' : 'text-gray-400 border border-white/10'
                                                }`}>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2">
                                                <button onClick={() => { setSelectedUser(user); setModalMode('edit'); setIsModalOpen(true) }} className="p-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg">
                            ...
                            {paginatedUsers.map((user) => (
                                <div key={user.id} className="bg-gray-800/50 rounded-xl border border-white/5 p-4 active:bg-gray-800 transition-colors">
                                    <div className="flex justify-between items-start mb-3">
                                        ...
                                        <button onClick={() => setMobileExpandedId(mobileExpandedId === user.id ? null : user.id)} className="text-gray-400">
                                    ...
                                    <div className="flex gap-2 mb-3">
                                        ...
                                        {user.is_superuser && (
                                            <span className="inline-flex px-2 py-0.5 rounded text-xs font-medium bg-purple-500/10 text-purple-400 border border-purple-500/20">Admin</span>
                                        )}
                                    </div>

                                    <div className="flex gap-2 border-t border-white/5 pt-3">
                                        <button onClick={() => { setSelectedUser(user); setModalMode('edit'); setIsModalOpen(true) }} className="flex-1 py-2 bg-gray-700 rounded-lg text-xs font-medium text-white">
                                            Edit / Details
                                        </button>
